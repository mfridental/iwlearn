...
from sklearn.metrics import accuracy_score, \
    mean_absolute_error  # keras metrics do not deliver a single value, but a tensor - not suitable for BaseModel.evaluate
...
class BaseKerasModel(BaseModel):
    ...
    def load(cls, filepath):
        ...
        if result.kerasmodelfile is not None:
            result.kerasmodel = k.models.load_model(result.kerasmodelfile, compile=True, custom_objects=result.custom_objects)
        return result
