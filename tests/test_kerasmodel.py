...
class TestFixture(object):
    def setup_class(cls):
        ...
        train_samples = [TestOfferSample.fromjson({'entityid': str(i), 'TestModelLabel': [0, 1] if i % 2 == 0 else [1, 0]}) for i in xrange(0, 10000)]
        test_samples = [TestOfferSample.fromjson({'entityid': str(i), 'TestModelLabel': [0, 1] if i % 2 == 0 else [1, 0]}) for i in xrange(10000, 12000)]
